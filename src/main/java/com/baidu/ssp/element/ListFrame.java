...
public class ListFrame implements Frame {
    ...
    public static Frame fromRows(List<Row> rows, List<String> titles) {
        if (rows == null || rows.isEmpty()) {
            return ListFrame.newInstance(Collections.<List<Object>>emptyList(), titles);
        } else {
            titles = rows.get(0).titles();
        }
    ...
    public List<String> titles() {
        return ImmutableList.copyOf(titles2Idx.keySet());
    }
    ...
    @Override
    public Frame rename(String src, String dest) {
        checkState(this.titles().contains(src));
        checkState(!this.titles().contains(dest));
        Integer index = this.titles2Idx.get(src);
        Map<String, Integer> newTitleMap = Maps.newHashMap(titles2Idx);
        newTitleMap.remove(src);
        newTitleMap.put(dest, index);
        return new ListFrame(data, newTitleMap);
    }
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = 0;i < this.height();i++) {
            ...
            Row otherRow = other.row(i).select(this.titles());
    ...
    private class ListRow implements Row {
        ...
        public Map<String, Object> mapView() {
            ...
            for (String title : titles) {
                result.put(title, fullRow.get(titles2Idx.get(title)));
            }
            return result;
        }
    }
