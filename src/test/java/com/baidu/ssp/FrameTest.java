...
public class FrameTest {
    ...
    public void testFilter() {
        assertEquals(Frames.filter(frame3, FramePredicates.onTitle("t1", Predicates.in(Arrays.asList(5)))),
                makeFrame(new String[]{"t2", "t1", "st"}, new Object[][]{
                        {5, 5, "10.00"}
                }));
        assertEquals(Frames.filter(Frames.limit(frame3, 0, 0),
                        FramePredicates.onTitle("t1", Predicates.in(Arrays.asList(5)))),
                makeFrame(new String[]{"t2", "t1", "st"}, new Object[][]{}));
    }
    ...
    public void testTransform() {
        assertEquals(Frames.transform(frame3, "t1", new Function<Integer, Integer>() {
            @Override
